...
- [x] `GET /health` still returns ok.
- [x] `GET /v1/parties` returns page envelope; empty DB → `items=[]`.
- [x] `GET /v1/parties/{id}` 404 for unknown UUID.
- [x] Seeded party appears in list/get.
- [x] `GET /v1/flags` returns only `status=open` rows.
- [x] `GET /v1/sources` defaults to approved (or empty).
- [x] No POST/PUT/PATCH/DELETE under `/v1` (router is GET-only).
- [x] Handlers use parameterized SQL / existing services (no string-built queries from input
      except bound params); issue only SELECT statements.
- [x] Public party/flag/source JSON does **not** include `meta`, `added_by`, `approved_by`,
