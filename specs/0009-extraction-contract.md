...
- **Epic / Issue**: E4.1 / #27 (contract) — also seeds new E4 stories (routing+Magika, confidence schema)
- **Status**: Draft
- **Author**: agent
- **Needs human decision?**: yes — extraction contract is `[H]` in `ROADMAP.md`; PDF-table engine
  choice (Docling vs pdfplumber/Camelot vs external sidecar) is an open ADR (see §7).
...
Two design lessons carried over from a prior project (`WealthOS/HMGCC`) shape this spec:

1. **Structure before semantics + confidence tagging.** Deterministic parsers (spreadsheets, JSON,
   PDF text-layer tables) are exact and cheap — treat their output as high-confidence `extracted`.
   LLM/OCR/vision output is probabilistic — tag it `inferred` (with a score) or `ambiguous`, never
   silently mixed with deterministic facts.
...
- **In scope**
  ...
  - Deterministic Pass 1 parsers wiring: XLSX/CSV, JSON, PDF text-layer tables.
...
- **Non-scope (own stories)**
  ...
  - The specific high-fidelity PDF-table engine (Docling etc.) — pending ADR (§7).
...
document (archived bytes + declared format)
  ...
  → 2. Pass 1 — deterministic (structure)
      ...
      └─ pdf(text) → text blocks + tables    derivation=extracted, confidence=1.0
...
- **Deterministic-first is a hard rule.** Pass 2 runs only when Pass 1 yields nothing usable for a
  document (e.g. a scanned PDF with no text layer). We never "upgrade" a deterministic result with an
  LLM guess. This keeps the cheap, exact, auditable path dominant and reserves spend/risk for the
  genuinely unstructured tail.
- **Quarantine, don't guess.** When Magika's top label is low-confidence, or contradicts the
  ...
  state and surfaced for human triage instead of being force-parsed. Mirrors PeaDF's sidecar
  `likely_scan` guard — refuse rather than emit misleading output.

### 3.2 Confidence tiering (derivation)

| derivation  | meaning                                   | source                              | default confidence |
|-------------|-------------------------------------------|-------------------------------------|--------------------|
| `extracted` | exact, reproducible, no model judgement   | spreadsheet/JSON/PDF-text parsers   | `1.0`              |
...
- [ ] A text-layer PDF fixture yields ≥1 table block with `page` + `region` provenance.
...
- [ ] A scanned-PDF fixture (no text layer) routes to Pass 2 and is tagged `inferred` (not `extracted`).
...
- **PDF-table engine (ADR pending human decision).** Options: (a) `pdfplumber` `extract_tables`
  (already proven in PeaDF's `pdf2xlsx_runner.py`, pure-Python, light) for text-layer PDFs as Pass 1;
  (b) **Docling** (MIT, layout + TableFormer, richer structure + native bbox provenance) as a
  higher-fidelity Pass-1.5; (c) reuse PeaDF's external Fly.io convert-sidecar.
  **Recommendation (to confirm):** run table extraction *in-engine* — `pdfplumber` for the common
  text-layer case now, evaluate Docling behind the same `extract_document` contract via an ADR; do
  **not** couple NaijaLedger ingestion to PeaDF's consumer sidecar (see the discussion accompanying
  this spec — provenance/WORM locality, decoupling, and runtime fit all favour in-engine).
